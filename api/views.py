...
#####################################################################
################  Ajouter des invités à un Moment PUBLIC (ATTENTION PAS LOGIN) ###################
######################################################################
# Methode acceptées : POST
# Paramètres obligatoires :
#	- idMoment, array de User
    ...
    if "users" in request.json:
        ...
        if moment is not None:
            if moment.privacy == constants.PUBLIC:
                ...
                response["error"] = "This moment is not public, in order to RSVP you have to be connected"
...
def assos_moments():
    ...
    moments_assos = sorted(moments_assos, key=lambda moment: moment.nb_guest_coming(), reverse = True)
